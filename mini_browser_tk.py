...
(moteur Tkhtml : HTML 4.01 / CSS 2.1 seulement).
Le JavaScript N'EST PAS supporté.

LIMITE CONFIRMÉE PAR TEST : les sites au CSS moderne/lourd (Wikipedia
avec son skin Vector 2022, Google, réseaux sociaux, apps web...) ne
fonctionnent pas et peuvent faire planter la fenêtre. Ça fonctionne
bien en revanche pour : sites simples, docs, pages HTML basiques,
blogs peu stylés.

Installation (Windows, cmd ou PowerShell) :
    pip install customtkinter tkinterweb pillow
...
# Page d'accueil compatible avec le moteur (testée et fonctionnelle).
# Change-la si tu veux, mais privilégie des sites au CSS simple.
HOME_URL = "https://tkinterweb.readthedocs.io/en/latest/index.html"

# Passe à True pour voir le détail du chargement de chaque page
# (requêtes, images, erreurs) dans le terminal.
DEBUG = False

# Support JavaScript expérimental (nécessite : pip install pythonmonkey).
# Reste limité : le moteur CSS sous-jacent (Tkhtml) est basique, donc même
# avec JS activé, les sites modernes très stylés peuvent mal s'afficher
# ou planter. Utile surtout pour de petits scripts sur des pages simples.
...
class MiniBrowser(ctk.CTk):
    def __init__(self):
        ...
        # --- Zone de rendu HTML ---
        # messages_enabled/message_func : diagnostics dans le terminal (voir DEBUG en haut du fichier)
        self.html_frame = HtmlFrame(
            self,
            messages_enabled=DEBUG,
